...
export const OperationsTVPanel: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-4 sm:p-6 space-y-6 font-sans select-none relative overflow-hidden">
      ...
      {hasNewSalePulse && (
        <div className="absolute inset-0 bg-blue-500/10 border-4 border-blue-500 animate-pulse pointer-events-none z-50 transition-all rounded-3xl" />
      )}
      ...
      <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-4 flex flex-wrap items-center justify-between gap-4 shadow-2xl backdrop-blur-md">
        ...
        <div className="flex items-center gap-3">
          <div className="p-2.5 rounded-2xl bg-gradient-to-tr from-blue-600 to-blue-500 text-white shadow-lg shadow-blue-500/30">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              {hasNewSalePulse && (
                <span className="bg-blue-500 text-slate-950 text-[10px] font-black px-2 py-0.5 rounded-full uppercase animate-bounce flex items-center gap-1">
        ...
        <div className="flex items-center gap-4">
          <div className="bg-slate-950 border border-slate-800 px-4 py-2 rounded-xl text-right">
            <div className="text-xl font-extrabold text-blue-400 font-mono tracking-wider">
          ...
          >
            {settings.soundEffectsEnabled ? <Volume2 className="w-5 h-5 text-blue-400" /> : <VolumeX className="w-5 h-5 text-slate-500" />}
          </button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-gradient-to-br from-slate-900 to-slate-900/80 border border-blue-500/40 rounded-2xl p-5 shadow-2xl relative overflow-hidden">
          ...
          <div className="text-xs text-slate-400 flex items-center justify-between pt-2 border-t border-slate-800">
            ...
            <span className="text-blue-400 font-bold">HOJE</span>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-slate-900 to-slate-900/80 border border-blue-500/40 rounded-2xl p-5 shadow-2xl relative overflow-hidden col-span-1 sm:col-span-2">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
              <Target className="w-4 h-4 text-blue-400" />
              Meta de Vendas do Dia
            </span>
            <span className="text-xl font-black text-blue-300">{goalAchievedPercent}% Atingido</span>
          </div>

          <div className="w-full bg-slate-950 rounded-full h-4 p-0.5 border border-slate-800 mb-3">
            <div
              className="bg-gradient-to-r from-blue-600 via-blue-500 to-blue-400 h-full rounded-full transition-all duration-1000 shadow-md shadow-blue-500/20"
          ...
          <div className="flex items-center justify-between text-xs text-slate-300">
            ...
            <span>Realizado: <strong className="text-blue-400 font-mono">{formatCurrency(todayRevenue)}</strong></span>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
          {cnpjBreakdown.map((c) => (
            <div key={c.code} className="bg-slate-900/90 border border-slate-800 rounded-2xl p-4 text-center shadow-lg hover:border-slate-700 transition-all">
              ...
              <div className="text-xs font-extrabold text-blue-400 font-mono">{formatCurrency(c.val)}</div>
            ...
            {mktBreakdown.map((m) => (
              <div key={m.id} className="bg-slate-950 p-3 rounded-xl border border-slate-800/80 flex items-center justify-between">
                ...
                <div className="text-right">
                  <span className="text-sm font-black text-blue-400 font-mono">{formatCurrency(m.val)}</span>
          ...
          <div className="space-y-2.5">
            {topProductsToday.map((p, idx) => (
              <div key={idx} className="bg-slate-950 p-3 rounded-xl border border-slate-800/80 flex items-center justify-between">
                ...
                <div className="text-right">
                  <span className="text-xs font-black text-blue-400 font-mono">{formatCurrency(p.val)}</span>
